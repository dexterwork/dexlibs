package com.geek.texaspoker.setup;
...
/**
 * Created by Dexter on 2018/4/11.
 */

public class ZoomImageView extends ImageView {
    final static String TAG = ZoomImageView.class.getSimpleName();
...
//                            MLog.d(TAG, "dx: " + dx);
                            ...
                            if (listener != null) {
                                float size = getScreenWidth(getContext()) * 0.5f;
                                if (dx < -size) listener.toRight();
                                if (dx > size) listener.toLeft();
                            }
                        }
                    ...
                    // 放大缩小图片
                        ...
                        if (endDis > 10f) { // 两个手指并拢在一起的时候像素大于10
                            ...
                            if (scale < 1 && (zoomX <= imgWidth || zoomY <= imgHeight)) {
                                float sw = imgWidth / zoomX;
                                float sh = imgHeight / zoomY;
                                scale = sw < sh ? sw : sh;
//                                MLog.d(TAG, "scale: " + scale);
//                                scale=1;
                                lock = false;
                            } else {
                                lock = true;
...
// TODO: 2018/4/11
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    float zoomY = intrinsicHeight * val[Matrix.MSCALE_Y];
                    float zoomX = intrinsicWidth * val[Matrix.MSCALE_X];
                    if (zoomX <= imgWidth || zoomY <= imgHeight) {
                        float sw = imgWidth / zoomX;
                        float sh = imgHeight / zoomY;
                        float s = sw < sh ? sw : sh;
                        lock = false;
                        if (midPoint != null) matrix.postScale(s, s, midPoint.x, midPoint.y);
                    }
    ...
    private void makeImgCenter(float[] values) {
        ...
        float zoomY = intrinsicHeight * values[Matrix.MSCALE_Y];
        float zoomX = intrinsicWidth * values[Matrix.MSCALE_X];
//        MLog.d(TAG, "zoomY: " + zoomY + " /zoomX: " + zoomX);
        // TODO: 2018/4/11
        // 图片左上角Y坐标
    ...
    private void getImageViewWidthHeight() {
        ...
        vto2.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                imgWidth = getWidth();
                imgHeight = getHeight();
//                MLog.i(TAG, "imgHeight: " + imgHeight + "/imgWidth: " + imgWidth + "/intrinsicHeight: " + intrinsicHeight + "/intrinsicWidth: " + intrinsicWidth);
            }
        });
    }
